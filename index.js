$(document).ready(function(){
    ...
    $(document).on("click", ':button', function(){
        ...
        $('#contact_container').html('<div id="contact_container"></div>');

        for (var i = 0; i < con_list.length; i++){
            console.log(con_list[i]);
            $("#contact_container").append("<div class='contact_card'><p class='contact_card_face'> First: " + 
            con_list[i].first_name + "</br>Last: "  + con_list[i].last_name +  
            "</p>" + "<p class='contact_card_back'> Email: " + con_list[i].email +  "</br>Phone: " + con_list[i].phone + "</p></div>");
        }
    ...
    $(document).on('click', '.contact_card_face', function(){
